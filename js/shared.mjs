...
const shared = {
  ...
  //     resourcesPath = string (path to the app resources)
  //     versions      = object (versions of chrome, electron, node, v8)
  ...
  detectKeyboardModifiers (evt) {
    ...
    if (evt.metaKey) {
      if (shared.info.process.platform === "darwin") {
  ...
  errorString (err) {
    ...
    if (typeof error !== "string") {
      if (error.message) {
        error = `${err.name}: ${err.message} (${shared.errorReduceStack(err.stack)})`;
      } else {
        return "" + error;
      }
    }
