...
const hints = {
  ...
  async glean () {
    ...
    // glean hints data
    await hints.parseData();
    await hints.parseFiles();
  ...
  async parseData () {
        ...
        // SEMANTIC_TYPE: propose to add semantic types
          ...
          for (const x of target.links) {
            ...
            if (!semCorrCorrect(data.hl, semCorr, x.type)) {
              ...
              const verweistext = x.verweistext ? `<Verweistext>${x.verweistext}</Verweistext>` : "<Verweistext/>";
              const hidx = hints.hidxAttribute(x.verweisziel);
              await hints.add(target.hints, i.lemma.file, {
                line: x.line,
                linkCount: 0,
                scope: x.scope,
                textErr: [
    ...
    for (const file of files) {
      ...
      if (duplicate) {
        await hints.add(xml.data.files[file].hints, file, {
          line: 3,
          linkCount: 0,
          scope: "Artikel",
          textErr: [
  ...
  async parseFiles () {
    for (const [ file, content ] of Object.entries(xml.files)) {
      ...
      try {
        await hints.checkHeading(file, doc, content);
        await hints.checkEZ(file, doc, content);
        await hints.checkSW(file, doc, content);
        await hints.checkTR(file, doc, content);
        await hints.checkVE(file, doc, content);
        await hints.checkSprache(file, doc, content);
        await hints.checkDiasystems(file, doc, content);
        await hints.checkLiterature(file, doc, content);
        await hints.checkQuotations(file, doc, content);
        await hints.checkRevisions(file, doc, content);
        await hints.collectComments(file, doc, content);
      } catch (err) {
        xml.updateErrors.push({
          file,
  ...
  //   doc = document
  //   content = string
  async checkHeading (file, doc, content) {
  ...
  //   doc = document
  //   content = string
  async checkEZ (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  //   doc = document
  //   content = string
  async checkSW (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  //   doc = document
  //   content = string
  async checkTR (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  //   doc = document
  //   content = string
  async checkVE (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  //   doc = document
  //   content = string
  async checkSprache (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  //   doc = document
  //   content = string
  async checkDiasystems (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
    for (const lesarten of doc.querySelectorAll("Lesarten")) {
      for (const n of lesarten.childNodes) {
        ...
        await iterateNodes(n, n, getDiaValues(n));
      }
    }
    ...
    async function iterateNodes (nodes, lesart, diaValues) {
  ...
  //   doc = document
  //   content = string
  async checkLiterature (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  //   doc = document
  //   content = string
  async checkQuotations (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  //   doc = document
  //   content = string
  async checkRevisions (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
    ...
    const revs = [];
    const revisions = doc.querySelectorAll("Revisionen Revision");
  ...
  //   doc = document
  //   content = string
  async collectComments (file, doc, content) {
    const data = xml.data.files[file];
    if (!data) {
      return;
    }
  ...
  async add (h, file, obj) {
    ...
    const hash = await shared.sha1(ident);
  ...
  async fillDiasystems () {
    ...
    const path = await bridge.ipc.invoke("path-join", xml.gitDir, "share", "rnc", "Diasystematik.rnc");
    const exists = await bridge.ipc.invoke("exists", path);
    if (!exists) {
      return false;
    }
    const file = await bridge.ipc.invoke("file-read", path);
  ...
  async fillLiterature () {
    ...
    const path = await bridge.ipc.invoke("path-join", xml.gitDir, "share", "Literaturliste.xml");
    const exists = await bridge.ipc.invoke("exists", path);
    if (!exists) {
      return false;
    }
  ...
  async fillVariants () {
    ...
    const path = await bridge.ipc.invoke("path-join", shared.info.userData, "variants.json");
    ...
    if (save) {
      bridge.ipc.invoke("file-write", path, JSON.stringify(hints.variants));
    }
  },
};
