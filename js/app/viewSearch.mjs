...
const viewSearch = {
  ...
  async start () {
    ...
    if (document.getElementById("search-always-xml").checked) {
  ...
  searchXml () {
    ...
    res.replaceChildren();
    res.appendChild(misc.pleaseWait());

    // load worker if necessary
    if (!viewSearch.worker) {
      viewSearch.worker = new Worker("js/app/workerSearch.mjs", {
        type: "module",
      });
    ...
    // post data to worker
    const narrowSearch = document.getElementById("search-narrow");
    const regExp = [];
    ...
    viewSearch.worker.postMessage({
      ...
      regExp,
      sameLine: document.getElementById("search-same-line").checked,
      scope,
  ...
  printResults () {
    ...
    // no results
    if (!viewSearch.data.results.length) {
      res.appendChild(misc.nothingToShow());
      return;

    // too much results
    } else if (viewSearch.data.results.length > 5e3) {
      viewSearch.data.results.length = 0;
      const div = misc.nothingToShow("Zu viele Treffer!", "Tipp: Schränken Sie Ihre Suche weiter ein.");
      res.appendChild(div);
      return;
    }
  ...
  printMoreResults () {
    ...
    for (let f = start, len = viewSearch.data.results.length; f < len; f++) {
      ...
      if (i.file !== lastFile) {
        ...
        // create heading
        res.appendChild(misc.makeListHeading(i.file));
        // update variables
        lastFile = i.file;
      }
  ...
  async toggleAdvanced (force = "") {
    const a = document.getElementById("search-advanced");
  ...
  toggleAdvancedIcon () {
    ...
    if (!dataA["search-scope-0"] || checked > 1) {
      icon.src = "img/preferences-red.svg";
    } else {
      icon.src = "img/preferences.svg";
    }
  },
  ...
  toggleScope (cb) {
    const scope0 = document.getElementById("search-scope-0");
