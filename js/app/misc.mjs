...
const misc = {
  ...
  async menuCommand (command) {
    ...
    switch (command) {
      ...
      case "artikel-json":
        ...
        break;
      case "clusters":
        document.getElementById("view-clusters").click();
        break;
      case "error-log":
        error.showLog();
        break;
      case "filters":
        ...
        break;
      case "hints":
        document.getElementById("view-hints").click();
        break;
      case "overview":
        overview.show();
        break;
      case "preferences":
        ...
        break;
      case "search":
        await misc.viewToggle(document.getElementById("view-search"));
      ...
      case "update":
        ...
        break;
      case "xml":
        document.getElementById("view-xml").click();
        break;
    }
  },
  ...
  async loadXsl ({ obj, key, xsl }) {
    ...
    let resources = shared.info.process.resourcesPath;
    if (/node_modules/.test(resources)) {
      // app is not packaged => shared.info.process.resourcesPath is the path to the Electron resources
      resources = await bridge.ipc.invoke("path-join", shared.info.appPath, "resources");
    }
    const path = await bridge.ipc.invoke("path-join", resources, xsl);
    ...
    shared.error(`${result.name}: ${result.message} (${shared.errorReduceStack(result.stack)})`);
    return false;
  },
  ...
  nothingToShow (textWarn = "", textTip = "") {
    ...
    let tip = textTip || "";
    if (/hints|xml/.test(misc.view) && !Object.keys(xml.files).length) {
      tip = "Tipp: Klicken Sie auf <i>Update</i>, um die XML-Dateidaten zu laden.";
    } else if (!tip && misc.view === "search") {
  ...
  makeListHeading (file) {
    ...
    for (const icon of icons) {
      ...
      a.addEventListener("click", function (evt) {
        evt.preventDefault();
        misc[this.dataset.fun](this.dataset.file, this);
      });
  ...
  //   down = boolean
  scroll (down) {
    const topBars = document.getElementById("bar").getBoundingClientRect().bottom;
  ...
  async getFilterState () {
    ...
    if (/xml|clusters/.test(misc.view)) {
    ...
    if (a.id === "sorting-dir") {
      ...
      if (!a.classList.contains("active")) {
        for (const i of [ "alpha", "time" ]) {
          document.getElementById(`sorting-${i}`).classList.toggle("active");
        }
      } else {
        return;
      }
  ...
  sortingGetData () {
    return {
      ...
      filter: document.getElementById("sorting-filter").value.trim(),
  ...
  viewScrollTopReset (type) {
    if (type === "switched" && misc.viewScrollTop[misc.view]) {
      window.scrollTo(0, misc.viewScrollTop[misc.view]);
    } else if (!type) {
      window.scrollTo(0, 0);
    }
  },
  ...
  async viewToggle (button) {
    ...
    misc.viewScrollTop[misc.view] = window.scrollY;
    ...
    const sortingCont = document.getElementById("sorting-filter-cont");
    if (nextView === "hints") {
    ...
    let direction = [ 1, -1 ];
    if (views.indexOf(nextView) > views.indexOf(misc.view)) {
      direction = [ -1, 1 ];
    }
    ...
    await new Promise(resolve => {
      ...
      const sect = document.getElementById(misc.view);
    ...
    bars.filtersActive();
    misc.viewPopulate("switched");
    misc.switching = false;
  },
