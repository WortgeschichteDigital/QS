...
const tags = {
  ...
  show () {
    ...
    if (!Object.keys(tags.data).length) {
      ...
      const ttCont = document.getElementById("tags-tags");
      ttCont.replaceChildren();
      for (const i of tt) {
    ...
    [ "attributes", "nav", "label", "code" ].forEach(i => document.getElementById(`tags-${i}`).classList.add("off"));
  ...
  listAttributes (ele) {
    ...
    const attrCont = document.getElementById("tags-attributes");
    attrCont.replaceChildren();
    for (const i of attr) {
      const a = document.createElement("a");
  ...
  async showSummary (ele) {
    ...
    document.getElementById("tags-code").innerHTML = code.text;
  },
