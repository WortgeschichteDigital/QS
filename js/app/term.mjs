...
const term = {
  ...
  show () {
    overlay.show("term");
    document.getElementById("term-export").focus();
  },
  ...
  async exportFile () {
    ...
    const options = {
      ...
      defaultPath: await bridge.ipc.invoke("path-join", shared.info.documents, `terminologie.${type}`),
      filters: [
        {
    ...
    if (prefs.data.zdl) {
      options.defaultPath = await bridge.ipc.invoke("path-join", prefs.data.zdl, "root", "wb", "WGd", `terminologie.${type}`);
    }
    const result = await bridge.ipc.invoke("file-dialog", false, options);
    if (result.canceled || !result.filePath) {
      return;
    }
    const written = await bridge.ipc.invoke("file-write", result.filePath, file);
    if (written !== true) {
  ...
  async load (cli) {
    // Terminologie.json
    const path = await bridge.ipc.invoke("path-join", git.config.dir, "resources", "Terminologie.json");
    try {
      const content = await bridge.ipc.invoke("file-read", path);
      term.json = JSON.parse(content);
    } catch (err) {
      if (cli) {
        bridge.ipc.invoke("cli-message", "Error: reading Terminologie.json failed");
    ...
    if (!result) {
      if (cli) {
        bridge.ipc.invoke("cli-message", "Error: reading terminologie.tt failed");
        bridge.ipc.invoke("cli-return-code", 1);
      }
      return false;
    }
