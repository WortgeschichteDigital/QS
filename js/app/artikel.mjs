...
const artikel = {
  ...
  async show () {
    if (!artikel.data.calculating) {
      document.getElementById("artikel-calculating").classList.add("invisible");
    }
    await artikel.messages();
    overlay.show("artikel");
    document.getElementById("artikel-calculate").focus();
  },
  ...
  async messages () {
    ...
    for (const i of data) {
      const div = document.getElementById(`artikel-${i.type}`);
  ...
  async calculate ({ cli, noNew }) {
    ...
    const calc = document.getElementById("artikel-calculating");
    const calcImg = calc.firstChild;
    const calcText = calc.lastChild;
    calcImg.src = "img/loading.svg";
    ...
    // done
    calcImg.src = "img/button-yes.svg";
    ...
    const options = {
      ...
      defaultPath: await bridge.ipc.invoke("path-join", git.config.dir, "resources", "Artikel.json"),
      filters: [
        {
          name: "JSON",
          extensions: [ "json" ],
    ...
    const written = await bridge.ipc.invoke("file-write", result.filePath, JSON.stringify(artikel.data.json));
    if (written !== true) {
  ...
  async makeClusters (noNew) {
    ...
    // initialize worker
    artikel.worker = new Worker("js/app/workerClusters.mjs", {
      type: "module",
    });
