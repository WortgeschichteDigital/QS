...
export { clustersCheck as default };
...
const clustersCheck = {
  ...
  async build () {
    // prepare section
    const cont = document.getElementById("clusters-check");
    ...
    if (!c.length) {
      cont.appendChild(misc.nothingToShow("Keine Cluster gefunden!", "Tipp: Ändern sie die Filtereinstellungen."));
      return;
    }
    ...
    if (data.active === "preview") {
      ...
      for (const cluster of cRepo) {
        repoHashes.add(await clustersCheck.hash(cluster));
    ...
    for (let i = 0, len = c.length; i < len; i++) {
      ...
      if (data.active === "preview") {
        const hash = await clustersCheck.hash(c[i]);
  ...
  jump () {
    ...
    const barBottom = document.getElementById("bar").getBoundingClientRect().bottom;
    const validClusters = [];
