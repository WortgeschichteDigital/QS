...
const bars = {
  ...
  toggle (bar) {
    if (bar === "filters") {
      document.getElementById("fun-filters").classList.toggle("active");
    }
  ...
  filtersActive () {
    ...
    for (const [ k, v ] of Object.entries(data)) {
      ...
      if (nodeName === "INPUT" && (!offDefault && !v || offDefault && v) ||
          nodeName !== "INPUT" && v) {
        active = true;
        break;
      }
    }
    const button = document.getElementById("fun-filters");
    if (active) {
  ...
  filtersReset () {
    ...
    bars.filtersActive();
    if (/xml|hints|clusters/.test(misc.view)) {
      misc.viewPopulate();
    }
  },
  ...
  filtersToggleCat () {
    ...
    const hints = document.getElementById("filters-hints");
    if (misc.view === "xml") {
      status.classList.remove("off");
      hints.classList.add("off");
    } else if (misc.view === "hints") {
      status.classList.remove("off");
      hints.classList.remove("off");
    } else if (misc.view === "search") {
      status.classList.remove("off");
      hints.classList.add("off");
    } else {
  ...
  resultsHints () {
    ...
    // hide queries block
    document.getElementById("results-queries").classList.add("off");
  ...
  resultsSearch () {
    ...
    const queries = document.getElementById("results-queries");
  ...
  //   data = array
  resultsXml (data) {
    const files = document.getElementById("results-files");
  ...
  resultsSearchNextQuery (a) {
    ...
    const topBarsHeight = document.getElementById("bar").getBoundingClientRect().bottom;
  ...
  resultsSearchNextFile (a) {
    if (misc.view === "search" && viewSearch.data.running) {
      return;
    }
    ...
    // scroll to heading
    const topBarsHeight = document.getElementById("bar").getBoundingClientRect().bottom;
    const headingHeight = document.querySelector(`#${misc.view} h1`).offsetHeight;
  ...
  selectFill (select) {
    ...
    let icon = "";
    if (data.icon) {
      icon = `<img src="img/${data.icon}" width="24" height="24" alt="">`;
    }
  ...
  selectPopup (a) {
    ...
    for (const i of alle.concat(bars.filtersData[cat])) {
      ...
      let icon = "";
      if (i.icon) {
        icon = `<img src="img/${i.icon}" width="24" height="24" alt="">`;
      } else if (hasIcons) {
        icon = '<img src="img/placeholder.svg" width="24" height="24" alt="">';
      }
