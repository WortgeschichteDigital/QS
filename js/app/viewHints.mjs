...
const viewHints = {
  ...
  async populate (type) {
    ...
    // clear hints area
    const cont = document.getElementById("hints");
    ...
    if (!data.hints.length) {
      cont.appendChild(misc.nothingToShow());
      bars.resultsHints();
    ...
    // sort hints
    misc.sortingApply(misc.sortingGetData(), data.hints);
  ...
  print () {
    ...
    for (let f = start, len = viewHints.data.hints.length; f < len; f++) {
      ...
      if (i.file !== lastFile) {
        ...
        // create heading
        cont.appendChild(misc.makeListHeading(i.file));
        // update variables
        lastFile = i.file;
      }
  ...
  toggleMark ({ ident }) {
    ...
    document.querySelectorAll(`.hint-block[data-ident="${ident}"] a[data-fun="toggleMark"] img`).forEach(i => {
      i.src = `img/${viewHints.markIcons[state]}`;
    });
  },
  ...
  nav (down) {
    ...
    const barBottom = document.getElementById("sorting").getBoundingClientRect().bottom;
...
&lt;/Verweis&gt;--&gt;</span></code>`;
  ...
  async exportHints () {
    ...
    const options = {
      ...
      defaultPath: await bridge.ipc.invoke("path-join", shared.info.userData, "safe-replacements.json"),
      filters: [
        {
          name: "JSON",
          extensions: [ "json" ],
    ...
    const written = await bridge.ipc.invoke("file-write", result.filePath, JSON.stringify(exportList, null, " ".repeat(2)));
    if (written !== true) {
